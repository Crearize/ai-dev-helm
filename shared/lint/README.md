...
- **There is no score gate — not in these configs, and not downstream.** They deliberately set no failing threshold (`thresholds.break` / `mutationThreshold`). Mutation runs are proposed and executed via the `test-recommendation` skill (quality-check Step 5, propose-then-decide): it reads the generated report, presents the score as reference information only, and triages survivors with the user (`documents/development/quality-policy.md` §2 — no pass/fail score). Do not add a threshold to the config to "make it strict" — that invents a gate the policy deliberately does not have.
