...
- **The diff scope is changed lines (Stryker) or changed classes (PIT), never whole changed files.** `stryker.diff.config.mjs` derives mutation ranges from `git diff -U0 <base>...HEAD`; `pitest.gradle` narrows `targetClasses` to the changed sources when `-PmutationDiffBase=<ref>` is passed. An empty scope is reported and the run ends without mutating anything. The shipped mutator set is lean on purpose (non-behavioural mutators excluded, see quality-policy §2); re-enable one in a product-owned config that extends the shipped one, never by editing these package-managed files.
