...
export async function logsThenRethrows(): Promise<string> {
  try {
    return await risky();
  } catch (e) {
...
export async function usesLogger(): Promise<string> {
  try {
    return await risky();
  } catch (e) {
...
export function plainLogging(): void {
  console.error('startup diagnostics');
}

// OK: catch that logs and then returns a recovery value
export async function logsAndRecovers(): Promise<string> {
  try {
    return await risky();
  } catch (e) {
    console.error(e);
