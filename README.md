...
- **無条件ブロック**（フラグ・ブランチの状態に関わらず）: force / delete push（`-f`/`-d` の短縮オプション束ね形を含む）・`+refspec`・`--mirror`・`--all`・`--branches`、ゲート対象と同一行にある別の git 操作（判定対象は**閉じた集合**です。HEAD を動かす commit・reset・checkout・switch・cherry-pick・rebase・revert・am・bisect・update-ref・stash pop・stash apply は改行区切りの複数行にまたがってもコマンド全体で判定、fetch・branch -f/-d/-D/--force は同一行で判定。status・add・log・diff・tag・remote 等の同居はブロックしません）、`-C`・`--git-dir`・`--work-tree`・`--namespace`・`-c`・`--config-env`・`GIT_*=` 代入・同一行の `cd`/`pushd`、行内の展開文字（`$` `` ` `` `{` `%`。`$'…'`/`$"…"` の ANSI-C / ロケールクォートも展開扱い。`gh` の自由テキスト値オプション `-t`/`--subject`・`-b`/`--body`・`-F`/`--body-file` の3組6形を除く行内の全語が対象）、複数ゲート対象の同居、`<x>:main` 形の refspec（`<x>` が現在のブランチ名 / `HEAD` / `@`（大小無視）のいずれでもない逆形）。リダイレクト（`>` 等）はゲート判定上は特別扱いされず通常どおり解釈されます。別コマンドに分けて素の git を実行してください
- **分類予算**: 64 KB を超えるコマンド、または 1 コマンド内の git/gh 呼び出しが 256 を超える場合は分類しません。前者（64 KB 超）はゲート語（`merge`/`pull`/`push`/`rebase`/`pr`、`pulls/<n>/merge`）を**コマンド全体**（クォート `"`/`'` とバックスラッシュ `\` を除去した後の語でも判定）に含めば block、含まず展開文字（`$`/`` ` ``）も含まなければ allow です（同一行の別操作の判定は行いません）。除去後の文字列に展開文字を含む場合はゲート語の有無を問わず block します（`$'\x70'ush` 等を静的に読み切れない形への fail-closed）。後者（256 超の呼び出し数）と 1 MB を超える hook ペイロード（stdin 全体）は分類せず無条件 block します（読み切れない入力を allow にはしません）
