...
Step 3: ユニットテスト + テスト設計メモとの照合（High/Medium。test-design スキルのメモと突き合わせ、欠落時は遡及実行）
  ↓
Step 3.5: ミューテーションテスト（High: gate / Medium: advisory・差分スコープ = 変更行 / 変更クラス・ローカル実行 / 未導入・対象外・空スコープはスキップ記録）
  ↓
...
Step 3.5 では、High/Medium リスクの変更に対して差分スコープ（Stryker は変更行、PIT は変更クラス）のミューテーションテストを**ローカルで**実行し、テストが「実行されたか」ではなく「意図的な変異を検出できるか」を確認します。High は `gate`（生存ミュータントをトリアージし、調整後スコアと test-design メモに紐づく生存の有無で判定）、Medium は既定で `advisory`（1回実行して生存台帳を記録し、Step 4 の QA ペルソナへレビュー入力として渡す。ブロックしない）として動きます。ミューテーションテストのツールが未導入のプロダクト、および差分スコープが空の場合はブロックせずスキップを記録します。Low リスク、および変更領域別ステップ適用テーブルで Step 3（ユニットテスト）が対象外の領域（docs/infra のみ等）では Step 3 の照合・Step 3.5 とも実行しません。
