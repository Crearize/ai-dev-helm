...
describe('changed-ranges.mjs pure helpers (always runs)', () => {
  ...
  // The README's escape hatch for a root-level "!" file: a negated glob whose
  // "!" is escaped excludes just that file. compileMutateMatcher strips the
  // leading "!" itself and hands the rest to minimatch, which reads "\!" as a
  // literal "!" - pinned here because it is the only way around the loud
  // failure of deriveScope for such a path.
  it('a negated glob with an escaped "!" excludes exactly the root-level "!" file (README recipe)', () => {
...
describe('shipped PIT pitest.gradle shape (always runs)', () => {
  ...
  it('removes the stale PIT report before every diff run, including the disabled-task empty scope (#95)', () => {
    ...
    // The deletion target sits inside the register closure, and it is
    // guarded: reportDir is deleted recursively and a product may point it
    // anywhere, so anything outside the build directory must be refused.
    const deleteLine = withoutLineComments.indexOf('delete pitest.reportDir');
    ...
    expect(closure).toContain('GradleException');
