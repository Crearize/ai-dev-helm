...
describe.skipIf(!SMOKE)(
  ...
  () => {
    ...
    it(
      ...
      // 90 s per run below, 600 s for the whole test.
      { timeout: 600000 },
      () => {
        if (!fs.existsSync(STRYKER_BIN)) {
          throw new Error(
        ...
        try {
          ...
          execFileSync(
            ...
            { cwd: repo, encoding: 'utf8', timeout: 90000, stdio: 'pipe', env }
          );
          ...
          const runDiffIncremental = () => {
            const result = spawnSync(
              process.execPath,
              [STRYKER_BIN, 'run', 'lint/mutation/stryker.diff.config.mjs'],
              { cwd: repo, encoding: 'utf8', timeout: 90000, env: { ...env, MUTATION_INCREMENTAL: '1' } }
            );
            expect(result.status, result.stderr).toBe(0);
