...
describe('loadLintConfig', () => {
  ...
  // 4b. Malicious prototype-key check names (C1)
  it('does not crash or pollute Object.prototype on __proto__/constructor keys', () => {
    const repo = mkdir('repo');
    mkdir('repo', '.git');
    const configPath = path.join(repo, CONFIG_FILENAME);
    // Raw JSON so __proto__ is a real own key (JSON.parse keeps it literal)
    fs.writeFileSync(
      configPath,
      '{ "checks": { "__proto__": { "enabled": false }, "constructor": { "enabled": false }, "toString": { "enabled": false } } }',
      'utf8'
    );

    let result;
    expect(() => {
      result = loadLintConfig(repo);
    }).not.toThrow();

    // Object.prototype must be untouched
    expect({}.enabled).toBeUndefined();
    expect(Object.prototype.enabled).toBeUndefined();
    // The malicious names are reported as unknown, not applied
    expect(result.error).toBeNull();
    expect(result.warnings.length).toBe(3);
    expect(result.warnings.join(' ')).toContain('__proto__');
    // Real checks are unaffected and still enabled
    expect(result.checks['secrets'].enabled).toBe(true);
  });
