...
describe('import-exists check', () => {
  ...
  describe('tsconfig / jsconfig paths (C4)', () => {
    it('resolves @/* mapped to ./src/* via tsconfig', () => {
      write('tsconfig.json', JSON.stringify({ compilerOptions: { paths: { '@/*': ['./src/*'] } } }));
      write('src/util.ts');
      const violations = runOn('src/deep/app.ts', "import u from '@/util';\n");
      expect(violations).toEqual([]);
    });
    ...
    it('resolves ~/* mapped to ./src/* via tsconfig', () => {
      write('tsconfig.json', JSON.stringify({ compilerOptions: { paths: { '~/*': ['./src/*'] } } }));
      write('src/util.ts');
      const violations = runOn('src/app.ts', "import u from '~/util';\n");
      expect(violations).toEqual([]);
    });

    it('honors baseUrl when mapping tsconfig paths', () => {
      write(
        'tsconfig.json',
    ...
    it('lets an explicit alias override the tsconfig mapping', () => {
      write('tsconfig.json', JSON.stringify({ compilerOptions: { paths: { '@/*': ['./src/*'] } } }));
      write('lib/util.ts'); // only lib/ exists, not src/
      const violations = runOn('app.ts', "import u from '@/util';\n", {
        aliases: { '@/': 'lib/' },
      });
      expect(violations).toEqual([]);
    });

    it('reads jsconfig.json when tsconfig is absent', () => {
      write('jsconfig.json', JSON.stringify({ compilerOptions: { paths: { '@/*': ['./src/*'] } } }));
      write('src/util.js');
      const violations = runOn('src/app.js', "import u from '@/util';\n");
      expect(violations).toEqual([]);
    });

    it('does not crash on a malformed tsconfig', () => {
      write('tsconfig.json', '{ not: valid json,,,');
      let violations;
      expect(() => {
        violations = runOn('src/app.ts', "import u from '@/util';\n");
      }).not.toThrow();
      expect(Array.isArray(violations)).toBe(true);
    });
  });

  describe('prototype-chain, string literals, cache (C10 / P1)', () => {
    it('treats prototype keys like constructor as unknown packages', () => {
      write('package.json', JSON.stringify({ dependencies: {} }));
      const violations = runOn('src/app.js', "import c from 'constructor';\n");
      expect(violations).toHaveLength(1);
      expect(violations[0].message).toContain("'constructor'");
    });

    it('does not extract a specifier from a from-inside-a-string', () => {
      const violations = runOn(
        'src/app.js',
        'const note = "copied from \'legacy/foo\'";\n'
      );
      expect(violations).toEqual([]);
    });

    it('still flags a real bare import even beside a from-in-a-string', () => {
      write('package.json', JSON.stringify({ dependencies: {} }));
      const violations = runOn(
        'src/app.js',
        'const doc = "imported from \'legacy/x\'"; import x from \'ghost-pkg\';\n'
      );
      expect(violations).toHaveLength(1);
      expect(violations[0].message).toContain("'ghost-pkg'");
    });

    it('populates and reuses a provided cache', () => {
      write('src/util.js');
      write('src/app.js', "import u from './util';\n");
      const cache = { kind: new Map(), manifest: new Map() };
      const ctx = makeCtx('src/app.js', "import u from './util';\n");
      ctx.cache = cache;
      importExists.run(ctx);
      expect(cache.kind.size).toBeGreaterThan(0);
    });
  });
