...
describe('init integration', () => {
  ...
  describe('quality-gate hook', () => {
    ...
    it('blocks git push when the quality-check flag is absent', () => {
      const out = runHook({ tool_input: { command: 'git push origin main' } });

      expect(JSON.parse(out)).toEqual({
        decision: 'block',
        reason: expect.stringContaining('Quality check not passed'),
      });
    });

    it('allows git push and consumes the flag when present', () => {
      const flag = path.join(tmpDir, '.quality-check-passed');
    ...
    it('detects push in chained and -C forms', () => {
      for (const command of [
        'cd sub && git push',
        'git -C /some/dir push --force-with-lease',
        'echo done; git push',
      ]) {
    ...
    it('ignores non-push commands and leaves the flag intact', () => {
      const flag = path.join(tmpDir, '.quality-check-passed');
      fs.writeFileSync(flag, '');

      for (const command of ['git status', 'npm test', 'echo "git pushover"']) {
        const out = runHook({ tool_input: { command } });
        expect(out).toBe('');
      }
