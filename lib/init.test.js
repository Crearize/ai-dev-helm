...
describe('init integration', () => {
  ...
  describe('quality-gate hook', () => {
    ...
    it('blocks when any chained push segment targets main', () => {
      ...
      const outStr = runHook({
        tool_input: { command: 'git push origin feat/x && git push origin main' },
      });
      expect(outStr).not.toBe('');
      expect(JSON.parse(outStr).decision).toBe('block');
    });
    ...
    it('blocks a merge+push chain on main when the merged branch is unchecked', () => {
      ...
      const outStr = runHook({
        tool_input: { command: 'git merge feat/x && git push origin main' },
      });
      expect(outStr).not.toBe('');
    ...
    it('does not exempt a harness-only diff that changes gate parameters', () => {
      for (const file of ['CLAUDE.md', 'AGENTS.md', '.cursorrules']) {
        ...
        const outStr = runHook({ tool_input: { command: 'gh pr merge 20' } });
        expect(outStr, `${file} must not be exempt`).not.toBe('');
        const out = JSON.parse(outStr);
        expect(out.decision).toBe('block');
        expect(out.reason).toContain('Quality check not passed');
      }
    ...
    it('does not exempt a gate-parameter change written in other markdown notations', () => {
      const docs = [
      ...
      for (const doc of docs) {
        ...
        const outStr = runHook({ tool_input: { command: 'gh pr merge 28' } });
        expect(outStr, `${JSON.stringify(doc)} must not be exempt`).not.toBe('');
        const out = JSON.parse(outStr);
        expect(out.decision).toBe('block');
        expect(out.reason).toContain('Quality check not passed');
      }
    ...
    it('invalidates the flag when a follow-up commit changes a gate parameter', () => {
      ...
      const outStr = runHook({ tool_input: { command: 'gh pr merge 23' } });
      expect(outStr).not.toBe('');
      const out = JSON.parse(outStr);
      expect(out.decision).toBe('block');
      expect(out.reason).toContain('Code changed');
    ...
    it('does not exempt a live override hidden behind a decoy comment marker', () => {
      ...
      const outStr = runHook({ tool_input: { command: 'gh pr merge 25' } });
      expect(outStr).not.toBe('');
      expect(JSON.parse(outStr).decision).toBe('block');
    });
    ...
    it('does not exempt a delimiter-only edit that activates a commented override', () => {
      ...
      const outStr = runHook({ tool_input: { command: 'gh pr merge 26' } });
      expect(outStr).not.toBe('');
      expect(JSON.parse(outStr).decision).toBe('block');
    });
    ...
    it('keeps the exemption when a value changes inside a still-commented block', () => {
      ...
      const doc = (high) =>
        ...
        `- mutation_budget_minutes: 15\n-->\n\n${filler}`;
      const g = initRepo();
      commitFile(g, 'CLAUDE.md', doc(80), 'harness template');
      g('update-ref', 'refs/remotes/origin/main', 'HEAD');
      g('checkout', '-b', 'chore/harness');
    ...
    // Fail-closed: the content of a harness config file whose path cannot be
    // handed to the shell safely is unknowable, so the exemption must not apply.
    it('does not exempt a harness config file whose path cannot be inspected', () => {
      const g = initRepo();
      g('checkout', '-b', 'chore/harness');
      commitFile(g, 'sub$dir/CLAUDE.md', '# rules\n', 'odd path');

      const outStr = runHook({ tool_input: { command: 'gh pr merge 24' } });
      expect(outStr).not.toBe('');
      expect(JSON.parse(outStr).decision).toBe('block');
    });
