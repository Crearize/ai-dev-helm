...
describe('init integration', () => {
  ...
  describe('ensureClaudeGateRegistration', () => {
    ...
    // L33 (quality-check cycle 2, round 4): the L23 quoted-content branch
    // (`["'][^"']*quality-gate\.cjs["']`) accepted ANY characters before the
    // filename inside the quotes, with no requirement that a path separator
    // (or nothing at all) immediately precede it - so these quoted decoys
    // read as "already registered" exactly like the unquoted M5 decoys
    // above, and a real gate registration was never appended.
    it.each([
      ['node "echo skipping quality-gate.cjs"'],
      ['node ".claude/hooks/not-quality-gate.cjs"'],
      ['"my node" "a quality-gate.cjs"'],
    ])('does not treat quoted decoy "%s" as a real gate invocation', (decoyCommand) => {
      const dest = writeSettings({
        hooks: {
          PreToolUse: [{ matcher: 'Bash', hooks: [{ type: 'command', command: decoyCommand }] }],
        },
    ...
    // L34 (quality-check cycle 2, round 4): Claude Code's own "all tools"
    // wildcard matcher is the literal string "*", which is not a valid
    // regular expression (`new RegExp('*')` throws "nothing to repeat") - so
    // it used to fall through to the exact-match fallback and read as
    // NOT Bash-scoped, letting init append a redundant duplicate
    // registration alongside a gate already covering every tool.
    it('treats matcher "*" (the all-tools wildcard) as already covering Bash calls (no duplicate registration)', () => {
      const dest = writeSettings({
        hooks: {
          PreToolUse: [{ matcher: '*', hooks: [{ type: 'command', command: gateCommand, timeout: 30 }] }],
        },
  ...
  describe('ensureGitignoreEntries', () => {
    it('creates .gitignore with workflow artifact entries when absent', () => {
      ...
      // `**/` so the entry also covers workspace packages that run Stryker
      // from their own directory (#99); the root-anchored spelling must not
      // ship any more.
      expect(content).toContain('**/reports/mutation/');
      expect(content.match(/^reports\/mutation\/$/gm)).toBeNull();
    });

    it('adds the workspace-wide **/reports/mutation/ entry next to a root-only reports/mutation/ line without touching it (#99)', () => {
      ...
      expect(content.match(/^reports\/mutation\/$/gm)).toHaveLength(1);
      expect(content.match(/^\*\*\/reports\/mutation\/$/gm)).toHaveLength(1);

      ensureGitignoreEntries(tmpDir);
      expect(fs.readFileSync(gitignorePath, 'utf8')).toBe(content);
    });

    it('treats a hand-edited **/reports/mutation/ line as already covering (no duplicate on re-init) (#99)', () => {
      const gitignorePath = path.join(tmpDir, '.gitignore');
      fs.writeFileSync(gitignorePath, '**/reports/mutation/\n');
    ...
    it('makes git ignore mutation artifacts inside workspace packages, not only at the root (#99)', () => {
      ...
      for (const rel of [
        ...
        'apps/web/reports/mutation/stryker-incremental.diff.json',
        'apps/web/reports/mutation/stryker-incremental.diff.scope.json',
        'apps/web/.stryker-tmp/sandbox/x.ts',
      ]) {
        const result = spawnSync('git', ['check-ignore', '-q', rel], { cwd: tmpDir });
        expect(result.status, `${rel} must be ignored`).toBe(0);
      }
    });

    // Test-design memo falsification 10: a CRLF .gitignore without a trailing
    // newline (a Windows editor's default). The appended block must start on
    // its own line, the existing lines must survive untouched, the CRLF
    // spelling of an existing entry must count as present, and git must
    // honour the result.
    it('appends cleanly to a CRLF .gitignore without a trailing newline and keeps existing lines intact', () => {
      execFileSync('git', ['init', '-q'], { cwd: tmpDir });
      const gitignorePath = path.join(tmpDir, '.gitignore');
      fs.writeFileSync(gitignorePath, 'node_modules/\r\n.stryker-tmp/\r\ndist');
      ...
      expect(content.match(/[^\r]\n/), 'a CRLF file must not receive bare LF lines').toBeNull();
      const lines = content.split(/\r?\n/);
      expect(lines.slice(0, 3)).toEqual(['node_modules/', '.stryker-tmp/', 'dist']);
      expect(lines).toContain('**/reports/mutation/');
      // The CRLF-terminated existing entry was recognised, not duplicated.
      expect(lines.filter((l) => l === '.stryker-tmp/')).toHaveLength(1);
      for (const rel of ['dist', 'apps/web/reports/mutation/mutation.json', 'apps/web/.stryker-tmp/x']) {
        const result = spawnSync('git', ['check-ignore', '-q', rel], { cwd: tmpDir });
        expect(result.status, `${rel} must be ignored`).toBe(0);
      }
    ...
    it('re-init of a product initialised before #99 appends only the **/ entry after the existing content, without a second header', () => {
      const gitignorePath = path.join(tmpDir, '.gitignore');
      // The pre-#99 shape: header + entries with the root-only spelling, then
      // a section the product added later.
      ensureGitignoreEntries(tmpDir);
      fs.writeFileSync(
        gitignorePath,
        fs.readFileSync(gitignorePath, 'utf8').replace('**/reports/mutation/', 'reports/mutation/') +
          '\n# product section\nbuild/\n'
      );
      ...
      expect(lines.filter((l) => l === 'reports/mutation/')).toHaveLength(1);
      expect(lines.filter((l) => l === '**/reports/mutation/')).toHaveLength(1);
      expect(content.match(/# ai-dev-helm/g)).toHaveLength(1);
      // Appended at the end - after the product's own section, not "next to"
      // the old line - and nothing else moved.
      expect(lines.indexOf('**/reports/mutation/')).toBeGreaterThan(lines.indexOf('build/'));
      expect(lines.indexOf('build/')).toBeGreaterThan(lines.indexOf('reports/mutation/'));
