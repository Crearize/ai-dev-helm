...
if (!eslintResolveError) {
  ...
  // Every custom rule file that gets a RuleTester block below registers here;
  // the completeness test then asserts this set covers the entire rules/ dir,
  // so adding a 4th rule file without a RuleTester block fails the suite
  // (QA M4 - the meta-test must not silently miss new rules).
  const TESTED_RULES = new Set();

  // --- harness/no-forwardref ------------------------------------------------
  describe('custom rule: no-forwardref', () => {
    TESTED_RULES.add('no-forwardref.js');
  ...
  describe('custom rule: one-component-per-file', () => {
    TESTED_RULES.add('one-component-per-file.js');
  ...
  describe('custom rule: export-at-definition', () => {
    TESTED_RULES.add('export-at-definition.js');
  ...
  // --- rules/ coverage completeness (QA M4) ----------------------------------
  // Enumerate every rule file shipped under rules/ and assert each one was
  // exercised by a RuleTester block above. A new rule dropped into rules/
  // without its own RuleTester block fails here, so the hand-written coverage
  // can never silently fall behind the shipped assets.
  describe('custom rule coverage completeness', () => {
    it('every .js file in rules/ has a RuleTester block', () => {
      const ruleFiles = fs
        .readdirSync(RULES_DIR)
        .filter((f) => f.endsWith('.js'))
        .sort();
      const tested = Array.from(TESTED_RULES).sort();
      expect(
        tested,
        `rules/ files without RuleTester coverage: ` +
          ruleFiles.filter((f) => !TESTED_RULES.has(f)).join(', ')
      ).toEqual(ruleFiles);
    });
  });
  ...
  describe('harness.config.mjs preset (fixture mini-project)', () => {
    ...
    const EXPECTED_RULE_IDS = [
      ...
      // security (A1/B1) - dynamic-exec + javascript: URL sinks
      'no-eval',
      'no-new-func',
      'no-script-url',
      // react (A1) - XSS-prone JSX sinks
      'react/no-danger',
    ...
    // Plain-JS (CommonJS) fixture: the base security group and no-console must
    // fire on .js too (architect M12 - plain JS was previously unguarded).
    const EXPECTED_JS_RULE_IDS = [
      'no-eval',
      'no-implied-eval',
      'no-new-func',
      'no-script-url',
    ...
    it(
      'violation.js triggers the base security group and no-console',
      { timeout: 90000 },
      async () => {
        const eslint = createLinter();
        const results = await eslint.lintFiles([
          path.join(FIXTURE_DIR, 'violation.js'),
        ]);
        expect(results).toHaveLength(1);
        const messages = results[0].messages;
        const fatal = messages.filter((m) => m.fatal);
        ...
        const seen = Array.from(new Set(messages.map((m) => m.ruleId)));
        for (const ruleId of EXPECTED_JS_RULE_IDS) {
          expect(
            seen,
            `expected rule ${ruleId} to fire on violation.js; saw: ${seen.join(', ')}`
          ).toContain(ruleId);
        }
      }
    );
    ...
    it(
      'ok.js yields zero errors and zero warnings (Node/browser globals wired)',
      { timeout: 90000 },
      async () => {
        const eslint = createLinter();
        const results = await eslint.lintFiles([
          path.join(FIXTURE_DIR, 'ok.js'),
        ]);
        expect(results).toHaveLength(1);
        expect(
          results[0].messages,
          `ok.js must be clean; got: ${JSON.stringify(results[0].messages, null, 2)}`
        ).toEqual([]);
        expect(results[0].errorCount).toBe(0);
        expect(results[0].warningCount).toBe(0);
      }
