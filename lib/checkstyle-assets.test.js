...
// --- ast-grep guard <-> preset module enumeration --------------------------------
// no-blanket-suppress-warnings-java.yml hard-codes the preset's module names
// (bare-name and misdirected-prefix detection need the list). The header
// comments in both files ask maintainers to keep them in sync; this test makes
// the drift mechanical: the set named in the YAML must equal the set of
// checking modules in checkstyle.xml (everything except the structural
// Checker/TreeWalker and the two suppression-plumbing modules).
const BLANKET_RULE_PATH = path.join(
  REPO_ROOT,
  'shared',
  'lint',
  'ast-grep',
...
describe('no-blanket-suppress-warnings-java module enumeration (always runs)', () => {
  it('names exactly the checking modules declared in checkstyle.xml', () => {
    const xml = fs.readFileSync(CONFIG_PATH, 'utf8');
    const yaml = fs.readFileSync(BLANKET_RULE_PATH, 'utf8');
    const declared = new Set(
      Array.from(xml.matchAll(/<module name="([^"]+)"/g), (m) => m[1]).filter(
        (name) => !STRUCTURAL_MODULES.has(name)
      )
    );
    // Every alternation group of the form (A|B|C) that mentions IllegalCatch is
    // one copy of the enumeration; all copies must agree with the preset.
    const groups = Array.from(
      yaml.matchAll(/\(([A-Za-z]+(?:\|[A-Za-z]+)+)\)/g),
      (m) => m[1].split('|')
    ).filter((names) => names.includes('IllegalCatch'));
    expect(groups.length, 'expected at least one module alternation in the rule').toBeGreaterThan(0);
    for (const names of groups) {
      expect(
        new Set(names),
        'module names enumerated in no-blanket-suppress-warnings-java.yml must match checkstyle.xml'
      ).toEqual(declared);
    }
  });
});
