...
//   H5  expansion in a command or subcommand word -> S40
...
describe('quality-gate classifier', () => {
  ...
  it('S39 reads gate words through quotes and backslashes over the byte budget', () => {
    ...
    // H5 over the budget: the expansion set here is the TOKENIZER's, so a
    // brace-expanded or percent-escaped gate word blocks even though quote
    // stripping leaves no gate word visible at all.
    for (const tail of [
  ...
  it('S40 blocks an expansion in a command or subcommand word', () => {
    // H5: bash expands braces, variables and command substitutions in the
    // COMMAND word too, so the candidate scan is hunting for the very word the
    // shell has not written yet. Every one of these ran the gated call while
    // the classifier found no candidate at all and allowed the line in silence.
    for (const command of [
      'git pus{h..h} origin main',
      'gi{t..t} push origin main',
      'git $(echo push) origin main',
      'git pus${e}h origin main',
      'git ${x:-push} origin main',
      'gh p{r..r} merge 1',
      'gh a{p..p}i -X PUT repos/o/r/pulls/1/merge',
      'gh api -X PUT repos/o/r/pul{l..l}s/1/merge',
      'gh api -X PUT repos/o/r/pulls/1/%6Derge',
    ]) {
      // The rule is ctx-independent, so it answers the same on either branch
      // and with a flag in hand - and it never resolves anything.
      for (const ctx of [makeCtx(withFlag()), onMain(withFlag())]) {
        const verdict = classify(command, ctx);
        expect(verdict.decision, command).toBe('block');
        expect(verdict.rule, command).toBe('2');
        expect(verdict.reason, command).toMatch(/command or subcommand word/);
        expectUntouched(ctx);
      }
    }

    // Characterization: over-detection in the fail-closed direction. A command
    // word is a command word whatever it names, so an expanded program path
    // blocks too - and the reason says what to write instead.
    expect(expectBlock('$HOME/bin/tool --flag', makeCtx()).reason)
      .toMatch(/Write the program name and subcommand literally/);

    // Controls: an expansion in any OTHER position still only blocks a line
    // that already holds a candidate, so ordinary work is untouched.
    for (const command of [
      'git status',
      'echo $HOME',
      'npm run $TASK',
      'git commit -m "$MSG"',
      'git log --format=%H', // `%` in an argument, not in a subcommand word.
    ]) {
      const ctx = onMain(withFlag());
      expectAllow(command, ctx);
      expectUntouched(ctx);
