...
function rewriteHeaders(chunk, a, b, rel, prevExists, curExists) {
  ...
  for (let i = 0; i < end; i += 1) {
    ...
    for (const source of [a, b]) {
      // Git folds the leading POSIX slash into its a/ or b/ prefix; keep
      // that separator when removing the absolute snapshot path.
      if (source !== '/dev/null') out = out.split(source.replace(/^\//, '')).join(rel);
    }
...
function buildContextPack({ dir, cycle, baseRef = 'origin/main', outDir }) {
  ...
  A('```');
  A('');
  A('自動計測: `measurements.json` はこの生成処理の所要時間と取得時点のディスク上の規則・hook のハッシュを記録する。実際にモデルが読み込んだ規則やレビュー全体の時間・トークンの証明ではない。');
  const contextPath = path.join(cycleDir, 'context.md');
  fs.writeFileSync(contextPath, `${L.join('\n')}\n`);

  // Measurement is generated by the helper, never hand-reported by the model.
  // Inventory is best-effort: a broken optional skill link/file must not turn
  // telemetry into a new gate. Existing context/path validation errors still
  // throw before reaching this point. Timing excludes this artifact's own I/O.
  let inventoryAtCapture = null;
  let inventoryError = null;
  const capturedAt = new Date().toISOString();
  try { inventoryAtCapture = inventoryHarness({ projectDir: root }); }
  catch (err) { inventoryError = err.message; }
  const measurementPath = path.join(cycleDir, 'measurements.json');
  fs.writeFileSync(measurementPath, `${JSON.stringify({
    schema_version: 1,
    operation: 'quality_context',
    clock: 'monotonic',
    elapsed_ms: Number(process.hrtime.bigint() - started) / 1e6,
    timing_scope: 'build_start_through_context_and_inventory_capture',
    captured_at: capturedAt,
    inventory_at_capture: inventoryAtCapture,
