...
function setupClaudeCode(projectDir, selectedStacks, fileOptions = {}) {
  ...
  if (dryRun) {
    ...
    ensureClaudeGateRegistration(settingsDest);
  }
...
function setupCodex(projectDir, selectedStacks, fileOptions = {}) {
  ...
  if (dryRun) {
    ...
    upgradeCodexHooksFile(hooksDest);
  }
 ...
 * top-level `hooks` key — a bare event key is silently ignored, #112) and
 * raise any quality-gate hook entry whose timeout is below the minimum.
 * User-added hooks and every other key are untouched; an unparsable file is
 * left alone with a warning (the user owns it).
...
function upgradeCodexHooksFile(hooksPath) {
  ...
  try {
    ...
    console.warn(`  warning: could not parse ${hooksPath}; leaving it unchanged`);
    return;
  }
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    console.warn(`  warning: ${hooksPath} is not a JSON object; leaving it unchanged`);
    return;
  }
  ...
  // yet" would fold legacy top-level event arrays into a hooks-shaped
  // object and reassign it over the original value, silently discarding
  // whatever `hooks` actually held. Bail out like an unparsable file instead.
  if (
    parsed.hooks !== undefined &&
    (parsed.hooks === null || typeof parsed.hooks !== 'object' || Array.isArray(parsed.hooks))
  ) {
    console.warn(`  warning: ${hooksPath} has a non-object "hooks" key; leaving it unchanged`);
    return;
  }
    ...
    for (const entry of entries) {
      ...
      for (const hook of hooks) {
        if (
          ...
          hook.command.includes('quality-gate.cjs') &&
 ...
 *
 * So: raise any quality-gate entry under `hooks.PreToolUse` that sits below
 * the minimum (or carries no timeout), and append the template's gate entry
 * when none is registered. Every other key, event and user hook is preserved;
 * a file whose shape this function does not understand is left alone with a
 * warning (the user owns it).
...
function ensureClaudeGateRegistration(settingsPath) {
  ...
  try {
    ...
    console.warn(`  warning: could not parse ${settingsPath}; leaving it unchanged`);
    return;
  }
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    console.warn(`  warning: ${settingsPath} is not a JSON object; leaving it unchanged`);
    return;
  }
  if (
    parsed.hooks !== undefined &&
    ...
    console.warn(`  warning: ${settingsPath} has a non-object "hooks" key; leaving it unchanged`);
    return;
  }
  const preToolUse = parsed.hooks ? parsed.hooks.PreToolUse : undefined;
  if (preToolUse !== undefined && !Array.isArray(preToolUse)) {
    console.warn(
      `  warning: ${settingsPath} has a non-array "hooks.PreToolUse"; leaving it unchanged`
    );
    return;
  }
  ...
  const gateHooks = (preToolUse || []).flatMap(entryHooks).filter(isQualityGateHook);
  for (const hook of gateHooks) {
    if (typeof hook.timeout !== 'number' || hook.timeout < MIN_GATE_TIMEOUT_SECONDS) {
      hook.timeout = MIN_GATE_TIMEOUT_SECONDS;
      timeoutRaised = true;
    }
  }
  ...
  if (gateHooks.length === 0) {
    // No gate anywhere in PreToolUse: take the registration straight from the
    // template so the shipped matcher/timeout stay the single source of truth.
    const templatePath = path.join(PACKAGE_ROOT, 'templates', 'settings.json.template');
    let template;
    try {
      template = JSON.parse(fs.readFileSync(templatePath, 'utf8'));
    } catch {
      console.warn(`  warning: could not parse ${templatePath}; left ${label} unchanged`);
      return;
    }
    ...
    if (templateEntries.length === 0) {
      console.warn(
        `  warning: ${templatePath} registers no quality-gate hook; left ${label} unchanged`
      );
      return;
    }
  ...
  if (!registered && !timeoutRaised) return;
