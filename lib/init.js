...
const GITIGNORE_ENTRIES = [
  ...
  // Local mutation-testing artifacts (quality-check Step 3.5): the Stryker
  // sandbox and the reports/incremental files. Committing the incremental
  // file would share stale mutant state between developers.
  '.stryker-tmp/',
  'reports/mutation/',
];
...
/**
 * Targeted upgrade for an existing hooks.json: raise any quality-gate hook
 * entry whose timeout is below the minimum. User-added hooks and every other
 * key are untouched; an unparsable file is left alone with a warning (the
 * user owns it).
...
function raiseQualityGateTimeout(hooksPath) {
  ...
  let changed = false;
  for (const entries of Object.values(parsed && typeof parsed === 'object' ? parsed : {})) {
