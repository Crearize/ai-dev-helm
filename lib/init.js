...
// message, a similarly-named backup file). Requires a `command`-type hook
// whose command contains a whitespace/path-delimited token ending in
// "quality-gate.cjs" (optionally preceded by "node ").
const QUALITY_GATE_COMMAND_RE = /(^|[\\/\s])(node\s+)?[^\s]*quality-gate\.cjs(\s|$)/;
...
// A hook-event matcher that plausibly covers Bash tool calls: unspecified
// (both tools treat a missing matcher as "all tools"), the literal "Bash",
// "^Bash$", or any other matcher string that mentions Bash (e.g. an
// alternation like "^(Bash|Read)$"). A registration scoped to some other
// tool only (`matcher: "Read"`) does not actually gate Bash calls and must
// not be counted as a real registration.
function matcherTargetsBash(matcher) {
  return (
    matcher === undefined || matcher === null || (typeof matcher === 'string' && matcher.includes('Bash'))
  );
}
...
function upgradeCodexHooksFile(hooksPath) {
  ...
  if (gateHooks.length === 0) {
    ...
    try {
      ...
      console.warn(`  warning: could not parse ${templatePath}; left ${hooksPath} unchanged`);
      template = null;
    }
    const templateEntries = template
      ? (template.hooks?.PreToolUse || []).filter((entry) => entryHooks(entry).some(isQualityGateHook))
      : [];
    if (template && templateEntries.length === 0) {
      console.warn(
        `  warning: ${templatePath} registers no quality-gate hook; left ${hooksPath} unchanged`
      );
 ...
 * calls (unspecified, "Bash", "^Bash$", or any matcher string mentioning
 * Bash — not e.g. "Read"), and (b) actually invokes quality-gate.cjs as a
 * script, not merely a command that mentions the filename. Every other key,
...
function ensureClaudeGateRegistration(settingsPath) {
  ...
  if (!registered && !timeoutRaised) return undefined;
