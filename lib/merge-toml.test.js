...
describe('mergeToml', () => {
  ...
  // Package-managed rules (reason prefixed "Blocked by ai-dev-helm:") must
  // follow the template across releases: hardened regexes reach existing
  // users, and renamed/retired template rules do not linger as orphans.
  it('refreshes package-managed rules from the template', () => {
    const target = path.join(tmpDir, 'config.toml');
    const template = path.join(tmpDir, 'template.toml');
    writeToml(target, {
      rules: [
    ...
    writeToml(template, {
      rules: [
        {
          name: 'block-rm-rf-root',
          match: { command_regex: 'rm\\s+(-[a-zA-Z-]+\\s+)*-rf\\s+/' },
          decision: 'deny',
    ...
    expect(result.rules[0].match.command_regex).toContain('(-[a-zA-Z-]+\\s+)*');
  });

  it('drops a package-managed rule whose name left the template (rename)', () => {
    const target = path.join(tmpDir, 'config.toml');
    const template = path.join(tmpDir, 'template.toml');
    writeToml(target, {
      rules: [
        { name: 'block-git-clean-fd', decision: 'deny', reason: 'Blocked by ai-dev-helm: git clean -fd' },
        { name: 'user-rule', decision: 'deny', reason: 'my own rule' },
      ],
    });
    writeToml(template, {
      rules: [
        { name: 'block-git-clean-force', decision: 'deny', reason: 'Blocked by ai-dev-helm: git clean force' },
      ],
    });

    mergeToml(target, template);

    const names = readToml(target).rules.map((r) => r.name);
    expect(names).toContain('block-git-clean-force');
    expect(names).toContain('user-rule'); // user rules always survive
    expect(names).not.toContain('block-git-clean-fd'); // stale managed orphan dropped
  });

  it('leaves a non-array rules key untouched instead of overwriting it', () => {
    const target = path.join(tmpDir, 'config.toml');
    const template = path.join(tmpDir, 'template.toml');
    fs.writeFileSync(target, '[rules]\ncustom = "table-not-array"\n', 'utf8');
    writeToml(template, {
      model: 'gpt-5.5',
      rules: [{ name: 'template-rule', decision: 'deny' }],
    });

    mergeToml(target, template);

    const result = readToml(target);
    expect(result.rules).toEqual({ custom: 'table-not-array' });
    expect(result.model).toBe('gpt-5.5'); // rest of the merge still applies
  });
