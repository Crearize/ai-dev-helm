...
`init` copies `lint/mutation/pitest.gradle` into the product; the lint-scaffolding skill does the wiring below. The snippet is a **PIT (Pitest) configuration** for Gradle products - a `pitest { }` extension block plus the two entry-point tasks `mutationFull` and `mutationDiff`. It is **manually verified** (it needs a JVM + a real Spring Boot project to execute); there is no automated PIT run in this repo (no-CI policy), and the shipped snippet is exercised against a real project in a one-time manual verification, like the ArchUnit template. The `pitest { }` block was verified that way; the `mutationDiff` diff-scope logic added later follows the same discipline and is structurally guarded by `lib/mutation-assets.test.js`. The stale-report clean-up on the diff path (`mutationDiffCleanReport`, #95) was execution-verified in a scratch Gradle 8.5 project (gradle-pitest-plugin 1.15.0, PIT 1.16.1, JUnit 5.10.1, JDK 21), each run started with a seeded stale `mutations.xml`:

- empty scope: the clean task ran, `pitest` was skipped, the stale XML was gone afterwards;
- one changed class: PIT rewrote the report (5 mutants, no trace of the seeded file);
- a changed interface (no mutation point): PIT logged `Skipping coverage and analysis as no mutations found` and no report remained;
- `reportDir` pointed outside `build/`, at `build/` itself, or at `build/reports`: the build failed with the guard's message and the directory (including another tool's files under `build/reports`) was left untouched; the plugin default stayed accepted;
- `--configuration-cache`: the empty-scope run stored an entry and the next run reused it, the clean task running both times; editing a class afterwards invalidated the entry (the git output is read through `providers.exec`, so it is part of the cache key) and the next run re-derived a one-class scope.
...
- For every diff run `failWhenNoMutations` is off and `targetTests` is pinned to the full base-package scope - PIT's default derives the test filter from `targetClasses`, which would silently drop every test class not named after the changed class and deflate the score (verified against PIT 1.16.1). In a multi-module build this pin is why `__BASE_PACKAGE__` must be the root shared by every module - see Wiring.
...
- **The report directory is deleted before every diff run** (`mutationDiffCleanReport`, a `Delete` task wired as a dependency of `pitest`). A diff run that produces no mutants writes no XML, so an earlier run's `build/reports/pitest/mutations.xml` would otherwise be read as this run's result. It is a task dependency, not a `doFirst`, so it also runs on the empty scope where the `pitest` task itself is disabled (a disabled task skips its own actions; its dependencies still run). After a diff run, a missing `mutations.xml` therefore means this run produced no mutants - never a leftover. The task deletes `reportDir` recursively, so it refuses (build failure) a `reportDir` that is not strictly below the module's build directory, or that is `build` / `build/reports` itself; keep `reportDir` PIT-specific (the plugin default `build/reports/pitest`). `mutationFull` does not depend on the clean task when invoked on its own (a single invocation naming both tasks runs one diff-scoped `pitest`, as before); since both entry points share `reportDir`, the next diff run also removes the previous full run's report. The Stryker twin removes its report only on the empty scope - Stryker always rewrites it on a non-empty scope, PIT does not.
