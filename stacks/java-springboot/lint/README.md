...
- An empty scope (no production class changed) disables the `pitest` task and completes immediately; changed classes with no mutation point under `DEFAULTS` (interfaces, constants-only classes) complete with zero mutants and no XML report. quality-check records `mutation.reason: "empty_scope"` for both cases. In a multi-module build wired per module (see Wiring), every module logs its own `[mutationDiff] base <ref>: N changed class(es) in scope` or `[mutationDiff] empty scope` line. The run is an empty scope only when **no module contributed any mutant** - every module either logged `empty scope` or completed with zero mutants; never read a single module's line as the overall verdict. The `test-recommendation` skill is the source for how quality-check turns those lines into `mutation.reason: "empty_scope"`.
