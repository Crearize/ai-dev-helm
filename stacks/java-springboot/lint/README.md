...
- `reportDir` pointed outside `build/`: the build failed with the guard's message and the directory was left untouched;
- `--configuration-cache`: the empty-scope run stored an entry and the next run reused it, the clean task running both times.
