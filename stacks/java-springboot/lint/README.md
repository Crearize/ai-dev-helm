...
- An empty scope (no production class changed) disables the `pitest` task and completes immediately; changed classes with no mutation point under `DEFAULTS` (interfaces, constants-only classes) complete with zero mutants and no XML report. quality-check records `mutation.reason: "empty_scope"` for both cases. In a multi-module build that applies the snippet from `subprojects { }`, every module logs its own `[mutationDiff] ... in scope` or `empty scope` line - the run is an empty scope only when **every** module reports one; never read a single module's line as the overall verdict.
