...
`init` copies `mutation/stryker.config.mjs`, `mutation/stryker.diff.config.mjs` and `mutation/changed-ranges.mjs` to the product's `lint/mutation/`. The base config is a pre-built Stryker config: `testRunner: 'vitest'`, source-only `mutate` globs (tests, `*.d.ts` and generated/build output excluded), a lean mutator set (non-behavioural mutators excluded - the config's exclusion list is the single source; `ignoreStatic` on), the `clear-text` / `json` / `html` reporters, and `incremental` on for the full run. The diff config extends it, narrows `mutate` to the **changed lines**, and keeps `incremental` off by default; the full-run cache is never read or written by the diff run. `MUTATION_INCREMENTAL=1` opts the re-measurement into a diff-only cache - see "Re-measurement" under Wiring for the mechanism and its limits. The diff run is what the `test-recommendation` skill (quality-check Step 5, propose-then-decide) uses.
...
`mutation:full` mutates the whole `mutate` set. `mutation:diff` diffs the **working tree** against the merge base of the base ref (`git diff -U0 <merge-base>` - uncommitted edits count, so line numbers always match the files Stryker actually mutates), turns every hunk into a Stryker mutation range (`src/file.ts:12-15`) and mutates only those lines - a one-line change in a large file yields the mutants of that line, not of the whole file. Files outside the `mutate` globs (tests, `*.d.ts`, generated output) never enter the scope even when they changed; the matching is done with minimatch, applied in Stryker's own order, so the diff scope is always a subset of the full run's scope. Paths containing glob-magic characters (Next.js dynamic routes: `[id]`, `[...slug]`) cannot carry a mutation range - those files fall back to whole-file scope via a character-class-escaped glob; plain `(group)` route segments keep their line ranges. A file whose name starts with `!` at the scan root cannot be expressed at all - Stryker would read the entry as a negation - so the run fails loudly instead (rename the file; excluding it from the mutate globs with the leading `!` escaped - `'!\\!name.ts'` as written in the JS config, the pattern value being `!\!name.ts` - is only a second choice: the diff run's own matcher honours it everywhere, but Stryker resolves patterns as paths before matching, so on Windows the backslash is read as a separator and `mutation:full` keeps the file); a `!` deeper in the path is literal and keeps its range. Adjacent hunks are never merged (a downstream adaptation proposed in #113): the scope is exactly the changed lines, and since `mutate` reaches Stryker through the config file there is no command-line length limit to work around - a wiring that passes `--mutate` on the command line is the product's own concern.

The base ref defaults to `origin/main`, then `origin/master`, then a local `main` / `master` (clones and worktrees without remote-tracking refs; a stale local trunk only moves the merge base back, which widens the scope and never narrows it). A local trunk that is the checked-out commit itself, or ahead of it, is skipped: its merge base would be HEAD and every committed change would silently fall out of the scope, so the run fails loudly instead (fetch the base branch, or set `MUTATION_BASE_REF` to the trunk quality-check diffs against). A remote-tracking trunk in the same position is kept - there really is nothing to measure against it. `MUTATION_BASE_REF=HEAD` is accepted for an ad-hoc look at the uncommitted changes only: the run warns on stderr that the merge base is HEAD itself and that this is not a gate measurement, and quality-check does not accept `HEAD` as `mutation.base_ref` (Step 1 diffs against the trunk, so such a run would score a narrower scope than the gate checks). The push/merge gate hook probes only the two remote-tracking refs - deliberately, its "no base ref" already means "no exemption". Products whose base branch is named differently set `MUTATION_BASE_REF` (for example `MUTATION_BASE_REF=origin/develop npm run mutation:diff` - POSIX shells only; the Windows forms are under "Environment notes" below); quality-check requires the same base as its own diff detection and records it as `mutation.base_ref`. When none of the four can be resolved (single-branch or shallow clone), `mutation:diff` fails loudly with a fetch hint (`git fetch origin main`) - it never degrades into a silent empty scope; quality-check records that failure as `mutation.reason: "scope_error"`. The old wiring's silent full-run fallback on shallow clones is gone: fetch the base ref or set `MUTATION_BASE_REF`.

**Empty scope**: when no changed line falls inside the mutate set, `mutation:diff` deletes a stale `reports/mutation/mutation.json` from a previous run together with the diff cache and its sidecar (below), prints `[mutation:diff] empty scope ...` and exits 0 without starting Stryker (Stryker itself would otherwise abort with "No tests were executed"). If the stale report cannot be removed (a locked file; a `jsonReporter.fileName` that is a directory or resolves outside the run directory is never deleted at all), the run exits 1 instead of claiming an empty scope - quality-check records `scope_error` - because the leftover would be read as this run's result; a cache or sidecar that cannot be removed only warns, the next run's guard discards it. Note the asymmetry with the PIT twin, which removes its report directory before **every** diff run: Stryker always rewrites the json report on a non-empty scope, PIT writes no XML when a scoped class has no mutation point. quality-check records `mutation.reason: "empty_scope"` for that case.
...
**Re-measurement (incremental)**: the first measurement of a change runs without a cache - run it with `MUTATION_INCREMENTAL` unset. After adding killing tests, re-measure with `MUTATION_INCREMENTAL=1 npm run mutation:diff` (POSIX shells; the Windows forms are under "Environment notes"). Only the values `1` and `true` opt in, exactly (no trimming, no case folding); anything else keeps incremental off. The diff run then keeps its own cache in `reports/mutation/stryker-incremental.diff.json` plus a sidecar (`stryker-incremental.diff.scope.json`) recording the base ref, merge base and scope that produced it, and Stryker re-runs only the mutants whose source or tests changed (`N of N mutant result(s) are reused` in its log). The cache is read only while the current scope still covers every line of the recorded one against the same merge base - a reverted change, a rebase, a moved base ref or a missing sidecar discards it first (`[mutation:diff] incremental: starting the diff cache`), so the report never carries mutants of lines that left the scope. An empty scope clears the cache as well. The sidecar is validated like untrusted input (format version, hex merge base, non-empty well-formed entries) and an unreadable cache (truncated, or not shaped like a Stryker report) counts as absent, so a hand-edited sidecar or a truncated cache discards rather than reuses. A cache that has to be discarded but cannot be removed disables the cache for that run (`[mutation:diff] incremental: disabled for this run`, `incremental: false`) rather than letting Stryker read it. This is a guard against staleness, not against tampering: whoever can write the cache can also write a matching sidecar, and `mutation.incremental` in the quality-check report is the compensating record. The diff run always writes to `reports/mutation/` regardless of the base config's `incrementalFile` (a product setting is ignored for the diff run on purpose - it must never alias the full-run cache), so keep the `**/reports/mutation/` gitignore entry even if the product relocates other reports; a differently-named directory that needs to be tracked can be re-included with a `!` rule below it (re-include the directory itself - git cannot re-include single files under an excluded directory). Do not run two `mutation:diff` processes concurrently in one checkout - the cache and sidecar are not locked. The guard compares line numbers, while Stryker relocates cached mutants by matching source content; after a heavy edit of the same file a cached mutant can in theory land on a line outside the numeric scope, so a survivor reported on an unchanged line means: re-run once without the flag (the `test-recommendation` skill defines how that extra run is counted and recorded).
...
- **TypeScript 7**: `@stryker-mutator/core` calls `ts.parseConfigFileTextToJson()` - removed in TypeScript 7 - from its tsconfig preprocessor and fails at startup with `TypeError: ts.parseConfigFileTextToJson is not a function` (observed with core 10 and TypeScript 7.0.2). The core declares no `typescript` dependency, so a package-manager override cannot pin an older compiler for it. First check whether the Stryker release you install has restored TypeScript 7 support. If not, the workaround has two steps: set `tsconfigFile` to a path that does not exist (the preprocessor then no-ops and never touches the TypeScript API); and, **if that tsconfig `extends` (or `references`) a file outside the package**, also add `/tsconfig.json` to `ignorePatterns` so the sandbox copy never sees it - the preprocessor's real job was rewriting those `extends` / `references` paths for the sandbox copy, and without this second step Vite/oxc fails every test with `[TSCONFIG_ERROR]` (the reporting monorepo extended a shared parent, so it needed both). A tsconfig with no outside `extends` or `references` stays in the sandbox and keeps serving its `paths` / `jsx` settings.
- **Windows and `MUTATION_BASE_REF` / `MUTATION_INCREMENTAL`**: the `VAR=value command` prefix only works in POSIX shells. In PowerShell run `$env:MUTATION_BASE_REF = 'origin/develop'; npm run mutation:diff` (the assignment outlives the command - `Remove-Item Env:MUTATION_BASE_REF` clears it). Inside a package.json script - which cmd.exe executes on Windows - use `cross-env MUTATION_BASE_REF=origin/develop stryker run lint/mutation/stryker.diff.config.mjs` with `cross-env` as a product devDependency. The same two forms apply to `MUTATION_INCREMENTAL=1` (a `mutation:diff:incremental` script with `cross-env` keeps it one command on every platform).
