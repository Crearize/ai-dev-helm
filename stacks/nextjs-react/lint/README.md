...
### Environment notes (pnpm, TypeScript 7, Windows, runtime-dispatch tests)

Observed while wiring the shipped assets into products. Each is handled in the product-owned config or tooling - never by editing the package-managed files.

- **pnpm / isolated `node_modules`**: Stryker's default plugin discovery (`plugins: ['@stryker-mutator/*']`) reads the directory five levels above the core's own file, which under pnpm's `.pnpm/` layout does not contain the runner, so the run fails with `Cannot find TestRunner plugin "vitest"`. Name the runner explicitly in the product-owned config: `appendPlugins: ['@stryker-mutator/vitest-runner']`.
- **TypeScript 7**: `@stryker-mutator/core` calls `ts.parseConfigFileTextToJson()` - removed in TypeScript 7 - from its tsconfig preprocessor and fails at startup with `TypeError: ts.parseConfigFileTextToJson is not a function` (observed with core 10 and TypeScript 7.0.2). The core declares no `typescript` dependency, so a package-manager override cannot pin an older compiler for it. First check whether the Stryker release you install has restored TypeScript 7 support. If not, the workaround is a **pair** that must be applied together: set `tsconfigFile` to a path that does not exist (the preprocessor then no-ops and never touches the TypeScript API), and add `/tsconfig.json` to `ignorePatterns` when that tsconfig `extends` a file outside the package - the preprocessor's real job was rewriting `extends` for the sandbox copy, and without it Vite/oxc fails every test with `[TSCONFIG_ERROR]`.
- **Windows and `MUTATION_BASE_REF`**: the `VAR=value command` prefix only works in POSIX shells. In PowerShell run `$env:MUTATION_BASE_REF = 'origin/develop'; npm run mutation:diff`. Inside a package.json script - which cmd.exe executes on Windows - use `cross-env MUTATION_BASE_REF=origin/develop stryker run lint/mutation/stryker.diff.config.mjs` with `cross-env` as a product devDependency.
- **Tests that reach the source at runtime rather than by import** (`SELF.fetch()` under `@cloudflare/vitest-pool-workers`, dispatch through a worker or server entry point): the vitest runner picks the tests to run per mutant from the static module graph, so a test that never imports the mutated file statically is silently left out - the mutant is reported `Survived` (or `NoCoverage`) even though that test would kill it. In the diff scope this shows up as false survivors, in the full run as a depressed score. Add a side-effect import of the source entry point to the helper module those tests import. `setupFiles` do not work for this - the selection does not read the setupFiles graph.
