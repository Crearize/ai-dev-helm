...
> **フェーズ1時点の注記**: `risk_level` / `lint_cycles` / `lint_abort_reason` / `mutation` / `test_design` は本スキーマ上の定義であり、`quality-check` SKILL.md 側の出力配線はフェーズ2で行う。それまでに生成されたレポートにこれらのフィールドが存在しないことは、レポート不正ではなく未配線を意味する。
>
