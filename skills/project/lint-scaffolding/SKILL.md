...
- ルールを弱めて通すことは認めない。グループ単位で不採用にし、理由を記録する（`lint/README.md` の規定。未使用ディレクトリの削除は記録済みオプトアウトとして可）
...
**本節がレビューガイドへの記載形式の定義である**（カタログ §4 が「フェーズ3の `lint-scaffolding` 実装で確定する」とした形式）。
