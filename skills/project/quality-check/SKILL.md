...
Step 0: ドキュメント更新の確認（feature-documentation）
  ↓
Step 1: 変更領域の判定 + リスクレベル判定（High/Medium/Low）+ 既存フラグの削除
  ↓
...
まず既存の `.quality-check-passed` および `.quality-check-report.json` が残っていれば削除する（前回の許可証・レポートを持ち越さない）。以降の各ステップがこのファイルへ書き込む記録は、ここで新規作成される同一ファイルへの追記マージである。
...
`test-design` スキルを実装前に実行済みの場合、メモ冒頭に記録された自己判定レベルと本判定を突き合わせ、食い違う場合は高い方を採用する。
...
本表は該当領域の**静的チェック**コマンドの選択規則である。変更領域別ステップ適用テーブルが別途指定するコマンド（infra の該当ビルドコマンド等）は、`lint:all` の有無に関わらず併せて実行する。領域テーブル指定コマンド（infra ビルド等）の失敗・修正は `lint_cycles` のカウント対象外（quality-policy §5 のサイクル単位は静的チェックコマンドの実行）。
...
3. **メモが存在しない場合はエラーにしない。** その場で `test-design` スキルを遡及実行する（手順は同スキルの SKILL.md「quality-check Step 3 との接続」の遡及ルール — 仕様・要件から書き起こす／実装コードから写さない）。洗い出した不足テストを補完してから先へ進む

Low リスクの変更、および領域テーブルの Step 3 欄が `-` の領域では照合を実行しない（後者は `out_of_scope` として記録する）。
...
| 領域テーブルの Step 3 欄が `-` の領域（docs のみ / infra のみ） | 実行しない。`reason: "out_of_scope"` を記録（`documents/development/quality-policy.md` §2「マトリクス優先順位原則」） |
...
**Lint 担保済み（`lint-scaffolding` のカバレッジマップで Lint 担保に割当済み）の項目は AI レビューの対象外とする**（`documents/development/static-check-standard.md` §4「Lint 担保済み原則」。決定的チェックで落ちるものを AI レビューで二重に扱わない）。カバレッジマップ（lint-scaffolding が生成する採否台帳）が存在しないプロダクトでは何も除外しない。
...
- QAエンジニア: 「テストが十分か」ではなく「この実装が間違っていることを証明するテスト・入力」を探してください。既知のテスト値へのハードコード、実装と同一ロジックの複製による期待値生成、意味のない assertion を検出対象に含めます。
...
- **優先度に関わらず、対応できるものは全て対応すること**（この規定は**本スキルの最終ゲート**に適用される。開発中のタスク単位レビュー（実装 ↔ レビューのループ）には `documents/development/quality-policy.md` §5.5 の重要度ゲーティングが適用され、本規定は持ち込まない）
...
Step 1 以降の各ステップで蓄積してきた`.quality-check-report.json`に最終結果フィールドを追記する。
