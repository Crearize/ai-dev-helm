...
│   ↓                                                                    │
│ Step 3.5: ミューテーションテスト（High/Medium: gate 既定・差分スコープ = 変更行 / 変更クラス・サイクルごと1回 / 未導入・対象外・空スコープはスキップ記録） │
│   ↓                                                                    │
│ Step 4: マルチペルソナ・レビュー（Step 2〜3.5 の結果を入力に含める。ペルソナ並列）      │
│   ↓                                                                    │
│ 統合指摘（Lint 残存・テスト失敗・閾値未達・生存ミュータント・ペルソナ指摘）の対応        │
│   ├── 高/中指摘・未達ゲートなし → サイクル終了、Step 5へ                         │
│   ├── 残っている & 上限未満 & 直前と同一の高指摘でない → 次サイクル（Step 2 から）     │
│   └── 上限到達 or 停滞 → ユーザー判断（受容 / 方針変更して追加サイクル / 中断）         │
└──────────────────────────────────────────────────────────────────────┘
  ↓
Step 5: E2Eテスト（最終確認）
  ├── 問題あり → 修正 → 影響範囲のみ再検証（サイクルには含めない）
  └── 問題なし → Step 5.5へ
  ↓
...
| リスクレベル | 以降のステップへの影響 |
|---|---|
| High / Medium | Step 3 でテスト設計メモとの照合を実施。Step 3.5（ミューテーションテスト）を実行対象とする（High / Medium とも既定 `gate`。モード・閾値は quality-policy §2） |
...
失敗したテストは本ステップ内で**即時に修正して再実行**する（Step 3.5 はテストが失敗したままでは起動できないため — quality-policy §5「サイクル内で即時に対応する工程」）。再実行でも失敗するテストは**高指摘**として統合指摘に加え、Step 3.5 はそのサイクルでは `aborted_reason: "tests_failing"` としてスキップする。期待値を実装に合わせて書き換える修正は禁止（3-3 参照）。
...
テストが「実行されたか」ではなく「意図的な変異を検出できるか」を測る。High / Medium とも既定で `gate`（通過判定あり）として実行する（Medium は `mutation_mode_medium` で `advisory`（記録と Step 4 へのレビュー入力のみ）/ `off` に変更可）。本ステップは**サイクルごとに1回**実行し、工程内の是正ループは持たない（quality-policy §5）。モードの定義・通過条件・トリアージの決定値・閾値は `documents/development/quality-policy.md` §2 を正とする（**数値・基準は本スキルに置かない**）。
...
| 上記以外（High / Medium かつツール導入済み） | 実行する。High は `gate`、Medium は `mutation_mode_medium` の宣言に従う（既定 `gate`） |
...
3. **`advisory` モードはここで終了する。** 判定・指摘化・ユーザー承認は行わず、台帳（`decision` は `untriaged` のままでよい）を Step 4 の QA エンジニアペルソナへ引き渡す
4. **`gate` モード**: 各生存ミュータントをトリアージする（決定値・カテゴリは quality-policy §2）。本ステップ内ではテストを追加しない。`equivalent` / `accepted` は理由（`accepted` はカテゴリも）を記録し、`memo_linked` の生存と振る舞いに影響する生存は `untriaged` のまま（前サイクルで対処済みでも殺せなかったものは `unresolved` として理由を記録）統合指摘の個別項目にする
5. quality-policy §2 の通過条件（調整後スコア ≥ 閾値 ∧ `untriaged` = 0 ∧ `memo_linked` の生存のうち `killed` / `equivalent` 以外が 0）で判定する。未達なら「ミューテーション通過条件未達」を**高指摘**として統合指摘に加える。テストの追加はサイクル末尾の統合指摘対応で行い、次サイクルの本ステップで再計測する（前サイクルの台帳を引き継ぎ、殺せた生存は `killed` に更新する）

- スコア閾値（リスクレベル別）・実行時間バジェット・バジェット超過時のスコープ絞り込みの基準は `documents/development/quality-policy.md` §2 / §5 を参照する（**数値は本スキルに置かない**）。閾値・バジェット・モードの上書きは同 §2「上書きの契約」に従う
- スコープを絞った場合は `mutation.scope_reduced` に記録する。バジェット超過で打ち切った場合は `aborted_reason: "budget_exceeded"` を記録し、`gate` モードでは高指摘として統合指摘に加える
...
- 統合指摘の対応中に Step 3.5 を**再実行しない**（再計測は次サイクルの本ステップで行う。同一サイクル内ではスコアと台帳を維持し、対処は `findings` に記録する）

### 記録

結果を `.quality-check-report.json` の `mutation` オブジェクト（`executed` / `reason` / `mode` / `scope` / `base_ref` / `mutants_total` / `score_raw` / `score` / `threshold` / `runs` / `survivors` / `scope_reduced` / `aborted_reason`）に記録する。複数サイクルで実行した場合は**最後の実行**の値を記録し、`runs` に実行回数を持つ（台帳は全サイクルを通じた最終状態）。フィールド定義は [`_schemas/quality-check-report.schema.md`](../_schemas/quality-check-report.schema.md) を参照。
...
サイクル上限（4-3 サイクルルール）まで通過条件未達または打ち切りが残った場合、サイクル全体の打ち切りとして部分結果・スコア・台帳を記録したうえで**ユーザーに判断を仰ぐ**（未達のまま通すか、方針を変えて追加サイクルを行うか、中断するか）。通す場合は**ユーザーの明示承認が必要**であり、承認を得たら `gate_override`（`steps` に `step_3.5`）に事実と理由を記録する。承認なしに `.quality-check-passed` を作成してはならない（quality-policy §5「打ち切り時のゲート挙動」。Step 6 の規定と同一）。`advisory` モードは判定を持たないため本節の対象外である（バジェット超過で打ち切った場合も `aborted_reason` を記録して先へ進む）。
...
## 前段工程の結果（このサイクルの Step 2〜3.5）
- 静的チェック: [残存違反の一覧。なければ「なし」]
- ユニットテスト: [失敗テストの一覧。なければ「全て成功」]
- テスト設計メモ照合: [status と不足件数]
- ミューテーション: [mode / 通過判定 / 調整後スコア / 閾値 / ミュータント総数。未実行ならその理由]
- 前サイクルの指摘と対応: [2サイクル目以降のみ。前サイクルの統合指摘と対応内容]
...
サイクルは Step 2 → 3 → 3.5 → 4 → 統合指摘の対応 を1周とする quality-check 全体のものであり、ペルソナレビュー単独のループは存在しない（quality-policy §5）。

- **統合指摘**には、ペルソナの指摘に加えて前段工程の結果を統合する: Step 2 の残存違反（高）、Step 3 の失敗テスト（高）、test-design 照合の持ち越し不足分（中）、Step 3.5 `gate` の通過条件未達・バジェット打ち切り（高）とその `untriaged` / `unresolved` 生存（個別項目）、QA ペルソナが台帳から指摘化した生存（ペルソナが付けた優先度）
...
- **統合指摘の対応後の判定**:
  - 高/中指摘が残っておらず、Step 3.5（`gate`）が通過条件を満たし `aborted_reason` もない → サイクル終了、Step 5 へ
  - それ以外で サイクル数 < 上限 → **次サイクル**（Step 2 から再実行。Step 3.5 は差分スコープ・incremental で再計測）。ペルソナは直前サイクルと同じ適用セットを用い、前サイクルの指摘と対応内容をプロンプト（「前段工程の結果」）に含める
...
**統合指摘に高/中指摘が残っておらず、Step 3.5（`gate`）が通過条件未達・打ち切りでなく、かつ最低1サイクル完了していること。** 上限到達・停滞で打ち切った場合はユーザー承認（`gate_override`）をもって完了とみなす。
...
- **問題あり**: 修正して影響範囲のみ再検証する（静的チェック・該当テスト・E2E の再実行。quality-check のサイクルには含めず、Step 2 からの再実行もしない）
- **問題なし**: Step 5.5へ進む
...
- [ ] High/Medium: 差分スコープ（変更行 / 変更クラス）でサイクルごとに1回実行した（Low・領域対象外・モード off・ツール未導入・空スコープはスキップ理由を記録）
- [ ] gate モード: 生存ミュータントをトリアージし、quality-policy §2 の通過条件で判定した。未達は高指摘として統合指摘に加えた（サイクル上限後も未達・打ち切りのまま通す場合はユーザー承認 + `gate_override` 記録）
- [ ] 生存台帳と前段工程の結果を Step 4 のペルソナ（台帳は QA エンジニア）へ引き渡した
- [ ] `mutation` オブジェクト（`runs` を含む）を記録した

### マルチペルソナ・レビュー（段階化・縮退で決まった適用ペルソナのみ）
- [ ] 差分規模を判定し適用ペルソナセットを決定した（生成ファイル除外済み）
- [ ] 適用した全ペルソナのレビュー完了
- [ ] 前段工程の結果（Lint 残存・テスト失敗・test-design 照合・ミューテーション結果）をペルソナのプロンプトに含めた
