...
- **優先度に関わらず、対応できるものは全て対応すること**（この規定は**本スキルの最終ゲート**に適用される。開発中のタスク実行時にはレビュー自体を行わない — `documents/development/quality-policy.md` §5.5「レビューの一本化」。品質レビューのループが存在するのは本スキルだけである）
