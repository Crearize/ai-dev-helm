...
Step 3: ユニットテスト（該当領域）
  ↓
Step 4: レビューサイクル（最低2回、指摘ゼロまで継続）
  ├── 4a: マルチペルソナ・レビュー（6ペルソナ並列）
  ├── 指摘あり → 修正 → Step 2に戻る
  └── 指摘なし & 2回以上完了 → Step 5へ
  ↓
Step 5: E2Eテスト（最終確認）
...
> docs/infraのみの変更ではStep 2, 3, 5がスキップされ、レビューサイクル2回とStep 5.75（self-improvement）が実行される。
...
| ハーネス | モデル指定方法 |
|---------|---------------|
| Claude Code | Taskツールの `model` パラメータに `fable` を明示指定（利用不可の場合は `opus`） |
| Codex | サブエージェントに `model = "gpt-5.5"` / `model_reasoning_effort = "high"` を明示指定 |
| Cursor | サブエージェント起動時に利用可能な最高精度モデル（Fable等。なければOpusクラス）を選択 |
...
Agentツールで6つのサブエージェントを**同時に**起動する。各サブエージェントには「使用モデル（必須）」のとおり最高精度モデルを明示指定する。
...
- **最低2サイクル実行**（指摘ゼロでも2回完走。拡充した6ペルソナで検出漏れを防ぐ）
...
**6ペルソナ全てから「必須修正事項（優先度: 高）」が0件であり、かつ2サイクル以上完了していること。**
