...
`test-design` スキルを実装前に実行済みの場合、メモ冒頭に記録された自己判定レベルと本判定を突き合わせる。**Step 1 の実差分に基づく判定を正とする。** test-design の自己判定より**下げる**場合は、その根拠を `.quality-check-report.json` に記録する（記録なしの引き下げは不可）。
