...
1 回の quality-check（単体実行時は 1 セッション）でのミューテーション実行は、**初回計測 1 回＋縮退再試行 1 回＋撃殺テスト追加後の再計測 1 回の最大 3 回**とする。超過にはユーザーの明示承認を要し、実行回数は `mutation.runs` に記録する（バジェットの数値・キーは従来どおり quality-policy §2 を参照する。縮退再試行が 1 回である旨も同 §2 を正とする）。
...
ミューテーションを実施し、`memo_linked: true` の生存または `equivalent` / `accepted` 判定が残った場合は、**QA エンジニアペルソナ 1 体**をサブエージェントとして起動し、台帳の分類妥当性を検証する（指摘が出ても非ブロック。quality-check から実行された場合はその指摘を記録に残し、対処はユーザーと合意する）。**実装者の自己トリアージだけで分類を確定しない。**
...
- **修正 → 再検証の反復は最大 2 回とする。** 2 回で緑にならない場合はユーザーに判断を仰ぐ — ① 失敗を受容してフラグを作成する（`gate_override` に記録）② 方針を変えて継続 ③ 中断。反復中はサーバーを起動したままにしてよい（停止は全反復完了後に 1 回・必須のまま）
...
- `scenario_added` / `dismissed` のまま 6 か月以上更新のない行は、末尾の「アーカイブ」節へ移してよい（判定対象から外れる）
...
- **ミューテーション側にも適用する**: 同一ブランチで `declined` 済みの同一対象領域は再提案しない（記録のみ更新する）。台帳の最終提示日・見送り回数を更新する
...
- [ ] 承認された E2E を実行した（server-startup で起動 → シナリオ実行 → 必ず停止。失敗は実バグとして修正 + 影響範囲のみ再検証 — 反復は最大 2 回、超過はユーザー判断。新規シナリオのテストコードはコミット）
